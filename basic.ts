 ...
 */
x = 42; // Impossible d'assigner le type 'number' au type 'string'
...
let zz: number;
zz = 41;
zz = "abc"; // Impossible d'assigner le type 'string' au type 'number'.
...
let aa: number[] = [];
aa.push(33);
aa.push("abc"); // L'argument de type 'string' n'est pas attribuable au paramètre de type 'number'.
...
bb = [1, 2, 3]; // Impossible d'assigner le type 'number' au type 'string'
...
cc = { 
  houseNumber: 33
}; // La propriété 'streetName' est absente du type '{ houseNumber: number; }' mais obligatoire dans le type '{ houseNumber: number; streetName: string; }'.
